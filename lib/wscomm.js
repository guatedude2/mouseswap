...
class WSComm {
    ...
    this.wss.on('connection', (ws) => {
      ...
      ws.on('message', jsonMessage((data) => {
        switch (data.action) {
          case 'AVAIL':
            ...
            this.events.emit('connected', data.data);
            break;
          default:
      ...
      ws.on('close', () => {
        if (ws.uuid) {
          ...
          this.events.emit('disconnected', ws.uuid);
        }
      });
    });
  ...
  connectToDevice(device) {
    ...
    ws.on('message', jsonMessage((data) => {
      switch (data.action) {
        case 'SWP':
          debug(`swap device ${data.data}`);
          if (this.device.uuid === data.data) {
            return this.events.emit('activate');
          }
  ...
  broadcastSwap(uuid) {
    ...
    debug('reporting swap to connected clients');
    this.wss.clients.forEach((ws) => {
      ws.send(JSON.stringify({ action: 'SWP', data: uuid }));
    });
    if (uuid === this.device.uuid) {
      return this.events.emit('activate');
    }
    this.events.emit('deactivate');
  }
  ...
  onActivate(callback) {
    this.events.on('activate', callback);
  }

  onDeactivate(callback) {
    this.events.on('deactivate', callback);
  }
